...
inline size_t getArraySize(T (&t)[sz])
{
  return sz;
}
...
VkDescriptorSetLayout NVK::createDescriptorSetLayout(const DescriptorSetLayoutCreateInfo& descriptorSetLayoutCreateInfo)
{
  ...
  (vkCreateDescriptorSetLayout(m_device, descriptorSetLayoutCreateInfo.getItemCst(), NULL, &dsl));
  return dsl;
}
//------------------------------------------------------------------------------
//
...
VkPipelineLayout NVK::createPipelineLayout(VkDescriptorSetLayout* dsls, uint32_t count)
{
  ...
  CHECK(vkCreatePipelineLayout(m_device, &pipelineLayoutCreateInfo, NULL, &pipelineLayout));
...
void NVK::CommandBuffer::cmdBindDescriptorSets(VkPipelineBindPoint    pipelineBindPoint,
...
{
  vkCmdBindDescriptorSets(m_cmdbuffer, pipelineBindPoint, layout, firstSet, descriptorSetCount, pDescriptorSets,
...
void NVK::CommandBuffer::cmdCopyImage(VkImage            srcImage,
...
void NVK::CommandBuffer::cmdBlitImage(VkImage            srcImage,
                                      VkImageLayout      srcImageLayout,
                                      VkImage            dstImage,
                                      VkImageLayout      dstImageLayout,
                                      uint32_t           regionCount,
...
void NVK::CommandBuffer::cmdCopyBufferToImage(VkBuffer                 srcBuffer,
                                              VkImage                  dstImage,
                                              VkImageLayout            dstImageLayout,
                                              uint32_t                 regionCount,
...
void NVK::CommandBuffer::cmdCopyImageToBuffer(VkImage                  srcImage,
                                              VkImageLayout            srcImageLayout,
                                              VkBuffer                 dstBuffer,
                                              uint32_t                 regionCount,
                                              const VkBufferImageCopy* pRegions)
...
void NVK::CommandBuffer::cmdClearColorImage(VkImage                        image,
...
void NVK::CommandBuffer::cmdClearDepthStencilImage(VkImage                         image,
                                                   VkImageLayout                   imageLayout,
...
void NVK::CommandBuffer::cmdClearAttachments(uint32_t                 attachmentCount,
...
void NVK::CommandBuffer::cmdResolveImage(VkImage               srcImage,
                                         VkImageLayout         srcImageLayout,
                                         VkImage               dstImage,
                                         VkImageLayout         dstImageLayout,
                                         uint32_t              regionCount,
...
void NVK::CommandBuffer::cmdWaitEvents(uint32_t                     eventCount,
...
{
  vkCmdWaitEvents(m_cmdbuffer, eventCount, pEvents, srcStageMask, dstStageMask, memoryBarrierCount, pMemoryBarriers,
                  bufferMemoryBarrierCount, pBufferMemoryBarriers, imageMemoryBarrierCount, pImageMemoryBarriers);
}
void NVK::CommandBuffer::cmdPipelineBarrier(VkPipelineStageFlags         srcStageMask,
                                            VkPipelineStageFlags         dstStageMask,
                                            ...
                                            const VkImageMemoryBarrier*  pImageMemoryBarriers)
{
  vkCmdPipelineBarrier(m_cmdbuffer, srcStageMask, dstStageMask, dependencyFlags, memoryBarrierCount, pMemoryBarriers,
                       bufferMemoryBarrierCount, pBufferMemoryBarriers, imageMemoryBarrierCount, pImageMemoryBarriers);
}
...
void NVK::CommandBuffer::cmdCopyQueryPoolResults(VkQueryPool        queryPool,
                                                 uint32_t           firstQuery,
                                                 uint32_t           queryCount,
                                                 VkBuffer           dstBuffer,
...
//
//------------------------------------------------------------------------------
VkBool32 dbgFunc(VkDebugReportFlagsEXT      flags,
...
bool NVK::utInitialize(bool bValidationLayer, WindowSurface* pWindowSurface)
{
  ...
  std::vector<std::string>              device_extension_name_storage;
  ...
  //if (1)
  {
    ...
    result                    = m_CreateDebugReportCallback(m_instance, &dbgCreateInfo, NULL, &m_msg_callback);
    switch(result)
    {
      case VK_SUCCESS:
        break;
  ...
  for(int j = 0; j < physical_devices.size(); j++)
  {
    ...
    LOGI("API ver. %x; driver ver. %x; VendorID %x; DeviceID %x; deviceType %x; Device Name: %s\n",
         m_gpu.properties.apiVersion, m_gpu.properties.driverVersion, m_gpu.properties.vendorID,
         m_gpu.properties.deviceID, m_gpu.properties.deviceType, m_gpu.properties.deviceName);
    ...
    {
      LOGI("Memory type %d: heapindex:%d %s|%s|%s|%s|%s\n", i, m_gpu.memoryProperties.memoryTypes[i].heapIndex,
           ...
           m_gpu.memoryProperties.memoryTypes[i].propertyFlags & VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT ?
               "VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT" :
               "");
    }
    for(unsigned int i = 0; i < m_gpu.memoryProperties.memoryHeapCount; i++)
    {
      LOGI("Memory heap %d: size:%" PRIu64 "Mb %s\n", i, m_gpu.memoryProperties.memoryHeaps[i].size / (1024 * 1024),
    ...
    for(int i = 0; i < device_extensions.size(); i++)
    {
      ...
      device_extension_name_storage.push_back(device_extensions[i].extensionName);
      device_extension_names[j][i] = device_extension_name_storage.back().c_str();
    }
  }
  //
...
void NVK::utFillImage(NVK::CommandPool* cmdPool, BufferImageCopy& bufferImageCopy, const void* data, VkDeviceSize dataSz, VkImage image)
{
  ...
  cmd.beginCommandBuffer(true, false);
  {
    ...
    cmd.utCmdSetImageLayout(image, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                            VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, subresourceRange);
    //obsolete: QueueAddMemReferences(m_queue, 1, &bufferStageMem);
    cmd.cmdCopyBufferToImage(bufferStage, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, bufferImageCopy.size(),
                             bufferImageCopy.getItem());
...
{
  VkSemaphoreCreateInfo semCreateInfo = {VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO, NULL, flags};
  VkSemaphore           sem;
...
{
  VkFenceCreateInfo finfo = {VK_STRUCTURE_TYPE_FENCE_CREATE_INFO, NULL, flags};
  VkFence           fence;
...
//
//------------------------------------------------------------------------------
void NVK::allocateDescriptorSets(const NVK::DescriptorSetAllocateInfo& allocateInfo, VkDescriptorSet* pDescriptorSets)
{
...
{
  return createBufferAlloc(BufferCreateInfo(size, bufferUsage, 0, VK_SHARING_MODE_EXCLUSIVE /*VkSharingMode sharingMode*/,
                                            0 /*uint32_t queueFamilyCount*/, NULL /*const uint32_t* pQueueFamilyIndices*/),
                           memProps, bufferMem);
}
//------------------------------------------------------------------------------
//
//------------------------------------------------------------------------------
VkBuffer NVK::MemoryChunk::createBufferAllocFill(NVK::CommandPool* cmdPool,
                                                 size_t            size,
                                                 ...
                                                 VkDeviceMemory*   bufferMem)
{
  VkResult result = VK_SUCCESS;
  VkBuffer buffer =
      createBufferAlloc(BufferCreateInfo(size, bufferUsage, 0, VK_SHARING_MODE_EXCLUSIVE /*VkSharingMode sharingMode*/,
                                         0 /*uint32_t queueFamilyCount*/, NULL /*const uint32_t* pQueueFamilyIndices*/),
                        memProps, bufferMem);
  if(bufferData)
  {
...
//-------------------------------------------------------------------------------------------
void NVK::CommandBuffer::utCmdSetImageLayout(VkImage                 image,
                                             ...
                                             VkImageSubresourceRange subresourceRange)
{
  ...
  vkCmdPipelineBarrier(m_cmdbuffer, srcStageFlags, destStageFlags, 0, 0, NULL, 0, NULL, 1, &imageMemoryBarrier);
}
