...
public:
  ...
  class iterator_impl : public std::iterator<std::forward_iterator_tag, T>
  {
    ...
    iterator_impl(V* b, size_t c, V* curPtr, bool freshB, bool freshE)
        ...
        , m_capacity(c)
        , m_ptr(curPtr)
        , m_isFreshBeginAndIsFull(freshB)
        , m_isFreshEndAndIsFull(freshE){};

  public:
    iterator_impl(const iterator_impl<V>& i)
        : m_buffer(i.m_buffer)
        , m_capacity(i.m_capacity)
        , m_ptr(i.m_ptr)
        , m_isFreshBeginAndIsFull(i.m_isFreshBeginAndIsFull)
        , m_isFreshEndAndIsFull(i.m_isFreshEndAndIsFull){};
    ...
    bool operator==(const iterator_impl<V>& rhs) const
    {
      if(m_ptr == rhs.m_ptr)
      {
        if((m_isFreshBeginAndIsFull && rhs.m_isFreshEndAndIsFull) || (m_isFreshEndAndIsFull && rhs.m_isFreshBeginAndIsFull))
    ...
    bool operator!=(const iterator_impl<V>& rhs) const { return !(operator==(rhs)); }
    V&   operator*() const { return *m_ptr; }
  ...
  iterator begin() { return iterator(m_buffer, m_capacity, m_readPtr, m_isFull, false); }
  ...
  const_iterator begin() const { return const_iterator(m_buffer, m_capacity, m_readPtr, m_isFull, false); }

  const_iterator end() const
  {
    if(m_capacity > 0)
      return const_iterator(m_buffer, m_capacity, (((m_readPtr - m_buffer) + GetStoredSize()) % m_capacity) + m_buffer,
                            false, m_isFull);
    else
      return begin();
  }
  ...
  void Reset() { Reset(GetCapacity()); }

  size_t GetFreeSize() const { return m_capacity - GetStoredSize(); }
  ...
  bool WriteData(const T* src, size_t len)
  {
    ...
    size_t rightFree = (m_writePtr >= m_readPtr && !m_isFull) ? (m_buffer + m_capacity) - m_writePtr :
                                                                m_readPtr - m_writePtr;  //stuff after the write pointer
    ...
    {
      std::allocator_traits<Alloc>::construct(m_allocator, &m_writePtr[i], src[i]);  //copy construct the objects
    }
    m_writePtr += writeAmt;
    ...
    if(len > writeAmt)
    {
      ...
      {
        std::allocator_traits<Alloc>::construct(m_allocator, &m_writePtr[i], src[i + writeAmt]);  //copy construct the objects
      }
  ...
  bool WriteData(const T& d) { return WriteData(&d, 1); }

  size_t ReadData(T* dest, size_t destSize) { return ReadDataImpl(dest, destSize, false); }

  bool ReadData(T& dest) { return ReadData(&dest, 1) > 0; }

  size_t ConsumeData(size_t count) { return ReadData(NULL, count); }
  ...
  size_t PeekData(T* dest, size_t destSize, size_t offset = 0) const
  {
    ...
    size_t rightStored = (m_writePtr < m_readPtr || m_isFull) ? (m_buffer + m_capacity) - m_readPtr :
                                                                m_writePtr - m_readPtr;  //stuff after the read pointer
  ...
  bool PeekData(T& dest, size_t offset = 0) const { return PeekData(&dest, 1, offset) > 0; }
...
private:
  ...
  size_t ReadDataImpl(T* dest, size_t destSize, bool construct)
  {
    ...
    for(size_t i = 0; i < readFromRightSize; i++)
    {
      ...
      {
        if(construct)
          std::allocator_traits<Alloc>::construct(m_allocator, &dest[i], m_readPtr[i]);
        else
          dest[i] = m_readPtr[i];
      }

      std::allocator_traits<Alloc>::destroy(m_allocator, &m_readPtr[i]);
    }
    ...
    if(readFromLeftSize > 0)
    {
      for(size_t i = 0; i < readFromLeftSize; i++)
      {
        ...
        {
          if(construct)
            std::allocator_traits<Alloc>::construct(m_allocator, &dest[i + readFromRightSize], m_readPtr[i]);
          else
